    ...
    for intento in range(1, 4):
        try:
            ...
            if len(respuesta.text.strip()) < 500:
                raise RuntimeError(
                    "Renta Corporación devolvió una página incompleta"
                )
...
def obtener_noticias(html):
    ...
    for enlace in soup.find_all("a", href=True):
        ...
        if not es_enlace_de_prensa(url):
            continue
    ...
    if not noticias:
        raise RuntimeError(
            "No se encontraron publicaciones en la Sala "
            "de Prensa de Renta Corporación"
        )
...
def crear_rss(noticias):
    ...
    partes = [
        ...
        '<rss version="2.0">',
        "<channel>",
        "<title>Renta Corporación - Sala de Prensa</title>",
        f"<link>{escapar_xml(WEB_URL)}</link>",
        (
            "<description>Últimas noticias y comunicados "
            "oficiales de Renta Corporación</description>"
        ),
...
def main():
    ...
    print(
        f"RSS de Renta Corporación actualizada: "
        f"{len(noticias)} publicaciones"
    )
